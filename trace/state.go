...
// Root returns the root of the current State. nil safe.
...
func ContextFromState(ctx context.Context, state *State) (context.Context, error) {
	...
	if state.TraceID != "" {
		...
		if err != nil {
			return ctx, fmt.Errorf("failed to parse TraceID, %w", err)
		}
	}
	if state.SpanID != "" {
		cfg.SpanID, err = trace.SpanIDFromHex(state.SpanID)
		if err != nil {
			return ctx, fmt.Errorf("failed to parse SpanID, %w", err)
		}
	}
